...
const SITE_CONFIG = {
  ...
  // Adjust these to control how fast/slow the loading screen feels.
  // stepDelays: when each step starts spinning (ms from start)
  // stepDones:  when each step shows a checkmark (ms from start)
  // confirmedAt: when "Address Confirmed" screen appears
  // advanceAt:   when it moves to the order form
  loader: {
    stepDelays:  [0, 1200, 2400, 3600, 4800],  // ← increase numbers = slower
    stepDones:   [900, 2100, 3300, 4500, 5700],
    confirmedAt: 6200,   // ← "Address Confirmed" appears at 6.2 seconds
    advanceAt:   7800,   // ← moves to order form at 7.8 seconds
  },

  /* ── ADD-ONS ─────────────────────────────────────────────── */
  // Add or remove add-ons here. Each shows as a checkbox on the order form.
  // id: unique key (no spaces), label: display name, desc: short description, price: number
  addons: [
    {
      id:    'listing_report',
